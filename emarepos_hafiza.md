...
### Derin Analiz Round 4 — 26 Bug Daha (commit c8ee9e6)

**Kritik — IDOR Güvenlik (8 adet):**
- **BUG-1:** OrderController show/updateStatus — branch_id kontrolü yoktu (şubeler arası sipariş erişimi) → eklendi.
- **BUG-2:** TableController open/detail/addOrder/pay/payPartial/transfer/updateTable/destroyTable (8 metod) — branch_id kontrolü yoktu → her metoda eklendi.
- **BUG-3:** TableController::transfer() hedef masa — `findOrFail` → `where('branch_id', session)->firstOrFail()`.
- **BUG-4:** TableController::payPartial() OrderItem::whereIn — branch filtresi yoktu → `whereHas('order', fn => where('branch_id'))` eklendi.
- **BUG-5:** CashRegisterController::report() ve saleItems() — branch_id kontrolü yoktu → eklendi.
- **BUG-6:** PurchaseInvoiceController show/update/destroy — branch_id kontrolü yoktu → eklendi.
- **BUG-7:** KitchenController — tenant kontrolü vardı ama branch_id yoktu → eklendi.
- **BUG-8:** ProductController::index() `orderBy` — kullanıcı kontrolünde sütun (SQL injection riski) → whitelist'e alındı.

**Kritik — Race Condition ve Atomiklik (6 adet):**
- **BUG-9:** SaleService::cancelSale() — zaten iptal satışı tekrar iptal edilebiliyordu (çift stok iadesi) → `in_array(status, [cancelled, refunded])` kontrolü eklendi.
- **BUG-10:** SaleService::createSale() satır döngüsü — `Product::find()` → `lockForUpdate()->first()` + stok yeterlilik kontrolü.
- **BUG-11:** CashRegisterService::openRegister() — transaction yoktu (çift kasa açılabiliyordu) → `DB::transaction + lockForUpdate` eklendi.
- **BUG-12:** StockTransferController::approve() — `DB::transaction` yoktu (yarıda kesilebilirdi) → eklendi.
- **BUG-13:** StockCountController::apply() — `DB::transaction + lockForUpdate` → eklendi.
- **BUG-14:** Firm/Customer addPayment() — `DB::transaction + lockForUpdate + refresh()` → eklendi.

**Yüksek — Mantık Hataları (3 adet):**
- **BUG-15:** PurchaseInvoiceController update/destroy — transaction yoktu → `DB::transaction + lockForUpdate` eklendi.
- **BUG-16:** CashRegisterService closeRegister gelir/gider filtresi — `date >= opened_at.toDateString()` gün başından filtrelerdi → `created_at >= opened_at` ile düzeltildi.
- **BUG-17:** CashRegisterController::salesDetail() credit filtresi — mixed+credit_amount>0 satışlar dahil edilmiyordu → `orWhere(payment_method=mixed AND credit_amount>0)` eklendi.

**Performans — Index Eksikliği (4 index):**
- **BUG-19:** `sales(branch_id, sold_at, status)` — composite index eklendi (migration 2026_03_09_000005).
- **BUG-20:** `orders(branch_id, ordered_at)` — index eklendi.
- **BUG-21:** `stock_movements(branch_id, movement_date)` ve `stock_movements(product_id)` — index eklendi.

**Performans — N+1:**
- **BUG-26:** DayOperationController::index() — 8+2=10 ayrı Sale/Order sorgusu → 3 DB::table sorgusu (selectRaw ile birleştirildi).

**Etkilenen Dosyalar (14):**
OrderController, TableController, KitchenController, CashRegisterController, PurchaseInvoiceController, ProductController, FirmController, CustomerController, DayOperationController, StockTransferController, StockCountController, SaleService, CashRegisterService, migration 2026_03_09_000005