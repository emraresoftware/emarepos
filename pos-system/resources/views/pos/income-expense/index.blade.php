...
<div x-data="incomeExpenseApp()" x-init="init()" class="flex-1 overflow-y-auto p-5 space-y-5" x-cloak>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div class="bg-white rounded-2xl p-5 border border-gray-100 shadow-lg shadow-gray-100/50">
            ...
            <div class="text-xl font-bold text-emerald-600">{{ number_format($totalIncome, 2, ',', '.') }} ₺</div>
        ...
        <div class="bg-white rounded-2xl p-5 border border-gray-100 shadow-lg shadow-gray-100/50">
            ...
            <div class="text-xl font-bold text-red-500">{{ number_format($totalExpense, 2, ',', '.') }} ₺</div>
            <div class="text-xs text-gray-400 mt-1">Seçili dönemde</div>
        </div>
        <div class="bg-white rounded-2xl p-5 border border-gray-100 shadow-lg shadow-gray-100/50">
            <div class="flex items-center justify-between mb-3">
                ...
                <div class="w-9 h-9 rounded-xl flex items-center justify-center
                    {{ $netBalance >= 0 ? 'bg-brand-50' : 'bg-amber-50' }}">
                    <i class="fas fa-scale-balanced text-sm {{ $netBalance >= 0 ? 'text-brand-500' : 'text-amber-500' }}"></i>
                </div>
            </div>
            <div class="text-xl font-bold {{ $netBalance >= 0 ? 'text-brand-600' : 'text-amber-600' }}">
        ...
        <div class="bg-white rounded-2xl p-5 border border-gray-100 shadow-lg shadow-gray-100/50">
            ...
            <div class="text-xl font-bold {{ ($thisMonthIncome - $thisMonthExpense) >= 0 ? 'text-purple-600' : 'text-red-500' }}">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-lg shadow-gray-100/50 p-4">
        ...
        <div class="flex gap-1 mb-4 bg-gray-100 rounded-xl p-1 w-fit">
            <button @click="activeTab = 'income'"
                ...
                <span class="ml-1 text-xs bg-emerald-100 text-emerald-600 px-1.5 py-0.5 rounded-full">{{ $incomes->total() }}</span>
            </button>
            <button @click="activeTab = 'expense'"
                ...
                <span class="ml-1 text-xs bg-red-100 text-red-500 px-1.5 py-0.5 rounded-full">{{ $expenses->total() }}</span>
            </button>
            <button @click="activeTab = 'types'"
                    :class="activeTab === 'types'
                        ? 'bg-white shadow text-brand-600 font-semibold'
                        : 'text-gray-500 hover:text-gray-700'"
                    class="px-4 py-1.5 rounded-lg text-sm transition-all duration-200">
        ...
        {{-- Tarih filtresi (gelir/gider sekmelerinde) --}}
        <div x-show="activeTab !== 'types'" class="mb-4">
            <form method="GET" action="{{ route('pos.income-expense') }}" class="flex flex-wrap items-center gap-2">
                ...
                <button type="submit"
                        class="px-3 py-2 bg-brand-500 hover:bg-brand-600 text-white rounded-xl text-sm font-medium transition-colors">
        ...
        <div x-show="activeTab === 'income'">
                        ...
                        <tr class="hover:bg-emerald-50/30 transition-colors">
                            <td class="px-4 py-3">
                                <span class="px-2.5 py-1 bg-emerald-100 text-emerald-700 rounded-full text-xs font-medium">
                                    {{ $income->type?->name ?? $income->type_name ?? '—' }}
                                </span>
                            </td>
                            <td class="px-4 py-3 text-gray-600 max-w-xs truncate">{{ $income->note ?: '—' }}</td>
                            <td class="px-4 py-3">
                                @php $pt = ['cash'=>'Nakit','card'=>'Kart','transfer'=>'Havale'][$income->payment_type] ?? $income->payment_type @endphp
                                <span class="text-xs text-gray-500">{{ $pt }}</span>
                            </td>
                            <td class="px-4 py-3 text-right font-bold text-emerald-600">{{ number_format($income->amount, 2, ',', '.') }} ₺</td>
                            <td class="px-4 py-3 text-gray-500 text-xs">{{ \Carbon\Carbon::parse($income->date)->format('d.m.Y') }}</td>
                            <td class="px-4 py-3 text-right">
                                <button @click="deleteRecord('income', {{ $income->id }})"
            ...
            <div class="mt-3">{{ $incomes->withQueryString()->links() }}</div>
            @else
                        ...
                        <tr class="hover:bg-red-50/30 transition-colors">
                            <td class="px-4 py-3">
                                <span class="px-2.5 py-1 bg-red-100 text-red-600 rounded-full text-xs font-medium">
                                    {{ $expense->type?->name ?? $expense->type_name ?? '—' }}
                                </span>
                            </td>
                            <td class="px-4 py-3 text-gray-600 max-w-xs truncate">{{ $expense->note ?: '—' }}</td>
                            <td class="px-4 py-3">
                                @php $pt = ['cash'=>'Nakit','card'=>'Kart','transfer'=>'Havale'][$expense->payment_type] ?? $expense->payment_type @endphp
                                <span class="text-xs text-gray-500">{{ $pt }}</span>
                            </td>
                            <td class="px-4 py-3 text-right font-bold text-red-500">{{ number_format($expense->amount, 2, ',', '.') }} ₺</td>
                            <td class="px-4 py-3 text-gray-500 text-xs">{{ \Carbon\Carbon::parse($expense->date)->format('d.m.Y') }}</td>
                            <td class="px-4 py-3 text-right">
                                <button @click="deleteRecord('expense', {{ $expense->id }})"
                                        class="text-gray-300 hover:text-red-500 transition-colors p-1">
                                    <i class="fas fa-trash text-xs"></i>
                                </button>
                            </td>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        @empty
                        <p class="text-sm text-gray-400 text-center py-4">Gelir türü yok</p>
                        @endforelse
                    </div>
                </div>
                {{-- Gider Türleri --}}
                <div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        @empty
                        <p class="text-sm text-gray-400 text-center py-4">Gider türü yok</p>
                        @endforelse
                    </div>
                </div>
            </div>
    ...
    return {
        ...
        async submitRecord() {
            ...
            try {
                ...
                await posAjax(url, { method: 'POST', body: JSON.stringify(this.form) });
        ...
        async deleteRecord(type, id) {
            ...
            try {
                const url = type === 'income'
                    ? `/income-expense/income/${id}`
                    : `/income-expense/expense/${id}`;
                await posAjax(url, { method: 'DELETE' });
        ...
        async submitType() {
            if (!this.typeForm.name) {
                showToast('Tür adı zorunlu', 'error');
                return;
            }
            try {
                const res = await posAjax('{{ route("pos.income-expense.type.store") }}', {
                    method: 'POST', body: JSON.stringify(this.typeForm)
                });
        ...
        async deleteType(id) {
            ...
            try {
                await posAjax(`/income-expense/type/${id}`, { method: 'DELETE' });
